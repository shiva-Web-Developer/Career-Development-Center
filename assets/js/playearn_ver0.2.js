    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(resp.isSaved == 'false'){
                ...
                if(resp.result < 3){
                    rows = '<span class="flex" style="margin: 0px 100px;font-size: 22px;color:red">You Lose</span><a class="btn-link text-center" style="margin: 15px 69px;">Education Points:'+resp.point+'</a><span id="res"></span><span class="text-center">'+resp.result+' of 5 questions answered correctly</span></p><button class="btn btn-primary" onClick="history.go(0);">Back</button><button class="btn btn-primary mt-2" onclick="history.go(0);">Play Again</button>';
                }else{
                    rows = '<span class="flex" style="margin: 0px 100px;font-size: 22px;color:green">You Won</span><a class="btn-link text-center" style="margin: 15px 69px;">Education Points:'+resp.point+'</a><span id="res"></span><span class="text-center">'+resp.result+' of 5 questions answered correctly</span></p><button class="btn btn-primary" onClick="history.go(0);">Back</button><button class="btn btn-primary mt-2" onclick="history.go(0);">Play Again</button>';
                }
