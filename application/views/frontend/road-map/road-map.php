        ...
        <div class="flex flex-row space-between flex-column">
            ...
            <div class="flex flex-row space-around">
                <div class="flex flex-column flex-content-center">
                    <div id="bmi-icon">
                        <img src="<?php echo base_url() ?>assets/icon/product.png">
                        <span>Road Map</span>
                    </div>
        ...
        <div>
            ...
            <div class="form-group">
                <div class="input-group mt-4 flex-content-center">
                    <input type="text" name="home-search" placeholder="Search here" id="home_search" onkeyup="myFunction()" title="Search here" autocomplete="off" required>
                    <button type"submit" class="btn btn-block btn-primary col-4 my-2" style="display:none" id="ser">Search</button>
                     <ul id="myUL">
                        ...
                        ?>
                            <li><a href="javascript:void(0)" onclick="searchtest('<?php echo $row['title'] ?>')"><?php echo $row['title'] ?></a></li>
                        <?php
                        }
                        ?>
                    </ul>
        ...
        foreach ($roadmap as $row) { ?>
            <div class="disease-card pt-1 flex">
                <div class="flex flex-column item-center" >
                    <img class="card-img-top pt-2" src="<?php echo base_url() ?>assets/img/roadmap/<?php echo $row['image'] ?>" id="course_img" alt="image">
                    <h6 class="card-title p-1" style="font-size:13px;"><?php echo $row['title']; ?></h6>
                    <a href="<?php echo base_url() ?>roadmapdescription?name=<?php echo $row['id']; ?>" class="btn btn-primary mt-1 text-light">View</a>
                </div>
            </div>
        <?php
        } ?>
    ...
    function searchtest(name) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('.cat').html('<div class="disease-card pt-1"><div class="flex flex-column item-center"><img class="card-img-top pt-2" src="<?php echo base_url() ?>assets/img/roadmap/' + data.img + '" alt="image"><h6 class="card-title">' + data.name + '</h6><a href="<?php base_url() ?>roadmapdescription?name=' + data.id + '"  class="btn btn-primary mt-1 text-light">View</a></div></div>');
                $('.cat').show();
                $('.pre').hide();
            }
        });
